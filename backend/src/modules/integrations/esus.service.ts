...
export class EsusService {
  ...
  parseCsv(content: string): EsusRow[] {
    ...
    if (streetIdx < 0) {
      throw new BadRequestException(
        'Coluna de logradouro não encontrada (rua, logradouro, no_logradouro…).',
      );
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const familyCount =
        familyIdx >= 0
          ? Math.max(0, parseInt(cols[familyIdx] ?? '0', 10) || 0)
          : 0;
      const inhabitantCount =
        inhabitantIdx >= 0
          ? Math.max(0, parseInt(cols[inhabitantIdx] ?? '0', 10) || 0)
          : 0;
  ...
  async importCsv(municipalityId: string, content: string, userId: string) {
    ...
    const result = await this.applyImport(
      municipalityId,
      items,
      userId,
      'esus-csv',
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (match.status === 'unmatched') {
        errors.push({
          ...
          message: 'Rua não encontrada no SIGAPS',
        });
        continue;
      }

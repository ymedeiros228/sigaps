...
describe('acs-street-coverage.util', () => {
  ...
  it('builds normalized variants without street prefixes', () => {
    expect(buildStreetCoverageVariants('Rua São José')).toEqual([
      'rua sao jose',
      'sao jose',
    ]);
  });
  ...
  it('builds searchable keys from street type and name', () => {
    expect(
      buildStreetSearchKeys({ name: 'São José', streetType: 'Rua' }),
    ).toEqual(['sao jose', 'rua sao jose']);
  });

  it('matchStreetRef resolves unique street', () => {
    const catalog = buildStreetRefCatalog([
      { id: 'a', name: 'Viriato', streetType: 'Rua' },
    ]);
